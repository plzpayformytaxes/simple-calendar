...
export default class MainApp extends FormApplication {
    ...
    render(force?: boolean, options?: Application.RenderOptions<FormApplicationOptions>): unknown {
        ...
        if (canUser((<Game>game).user, SC.globalConfiguration.permissions.viewCalendar)) {
            ...
            // Render using the parent class first
            const _renderResult = super.render(force, mergedOptions);
            // Ensure the outer element always has the legacy id so downstream logic relying on it still works.
            if (this.element && this.element.length) {
                this.element.attr("id", MainApp.appWindowId);
            }
    ...
    public static setWidthHeight(ma: MainApp) {
        let width = 0,
            height = 0;
        const main = (ma.element && ma.element.length ? (ma.element[0] as HTMLElement) : null) ||
            (<HTMLElement>document.querySelector(`#${MainApp.appWindowId}`));
        if (main) {
    ...
    public activateListeners() {
        ...
        // Prefer the jQuery element that Foundry attaches to the application; fall back to the document lookup.
        let appWindow: HTMLElement | null = null;
        if (this.element && this.element.length) {
            appWindow = this.element[0] as HTMLElement;
        }
        if (!appWindow) {
            appWindow = document.getElementById(MainApp.appWindowId);
        }
        if (appWindow) {
